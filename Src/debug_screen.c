...
//      }
      ...
      case 11: {
        for (uint16_t i = 0; i < 1000; i++) {
          DISP_FillRect(
              HAL_RNG_GetRandomNumber(&hrng) % DISP_getScreenWidth(),
              HAL_RNG_GetRandomNumber(&hrng) % DISP_getScreenHeight(),
              HAL_RNG_GetRandomNumber(&hrng) % DISP_getScreenWidth(),
              HAL_RNG_GetRandomNumber(&hrng) % DISP_getScreenHeight(),
      ...
      case SCREEN_MAX: {
        for (uint16_t i = 0; i < 5; i++) {
          DISP_FillScreen((uint16_t) HAL_RNG_GetRandomNumber(&hrng));
          HAL_Delay(750);
        }
        break;
      }
